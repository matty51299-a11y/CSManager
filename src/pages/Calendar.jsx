          ...
          <tbody>
            {events.map((e) => (
              <tr key={e.eventId}>
                ...
                <td style={{ color: 'var(--text-muted)', fontSize: 11 }}>{e.inviteMethod}</td><td>{gameState.activeEventId === e.eventId ? <button onClick={enterEvent}>Enter Event</button> : <button onClick={advanceToNextEvent}>Continue</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
