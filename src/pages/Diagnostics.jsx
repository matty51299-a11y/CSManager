...
export default function Diagnostics({ gameState, resetCareer }) {
  ...
  const careerChecks = [
    ...
    { name: 'Full-screen overlay renders active event', valid: eventActivePhases.includes(gameState.currentPhase) ? Boolean(gameState.activeTournament?.event) : true, errors: [] },
    { name: 'Dynamic VRS rankings initialize', valid: gameState.rankings.every((r) => r.vrsPoints && r.formRating), errors: [] },
    { name: 'Event invite list uses dynamic rankings', valid: gameState.currentEventId ? Boolean(gameState.eventInviteSnapshots?.[gameState.currentEventId] || gameState.rankings[0]?.vrsPoints) : true, errors: [] },
    { name: 'Event completion updates VRS points', valid: gameState.completedEvents[0]?.vrsPointDelta !== undefined || gameState.completedEvents.length === 0, errors: [] },
    { name: 'Ranks reorder after event', valid: gameState.rankings.every((r) => r.currentRank), errors: [] },
    { name: 'Rank movement is calculated', valid: gameState.rankings.every((r) => r.rankMovement !== undefined), errors: [] },
    { name: 'Form updates after wins/losses', valid: gameState.rankings.every((r) => r.formRating >= 0 && r.formRating <= 100), errors: [] },
    { name: 'Same event simulation has live variance unless fixed seed is used', valid: true, errors: [] },
    { name: 'Dashboard uses dynamic rank after event', valid: gameState.teams.every((t) => t.vrsPoints), errors: [] },
