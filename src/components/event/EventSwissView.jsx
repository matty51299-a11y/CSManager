...
export default function EventSwissView({ tournament, model }) {
  ...
  const userRecord = `${model.userStanding?.wins || 0}-${model.userStanding?.losses || 0}`;
  ...
  return <div className="swiss-focus-board">
    <div className="swiss-stage-card">
      ...
      <h2>Round {model.activeRound?.number || tournament.swiss.rounds.length + 1}: qualification race</h2>
      ...
      <div className="progress-lanes"><div><b>{tournament.swiss.qualified.length}</b><span>Qualified</span></div><div><b>{active.length}</b><span>Alive</span></div><div><b>{tournament.swiss.eliminated.length}</b><span>Eliminated</span></div></div>
    </div>
    <div className="pool-grid">
      ...
      <Pool title="Qualified" subtitle="playoff bound" teams={standings.filter((s) => s.status === 'qualified')} userTeamId={model.userTeamId}/>
      <Pool title="Eliminated" subtitle="event over" teams={standings.filter((s) => s.status === 'eliminated')} userTeamId={model.userTeamId}/>
    </div>
  </div>;
}
